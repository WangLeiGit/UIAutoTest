...
public class UIExecutorImpl implements UIExecutor {
	...
	public void appSwipeDown(int duration, int num) {
		...
		int height = appiumDriver.manage().window().getSize().getHeight();
		for (int i = 0; i < num; i++) {
			appiumDriver.swipe(width / 2, height / 4, width / 2, height * 3 / 4, duration);
			sleep(1000);
		}
	}
	...
	public void appSwipeLeft(int duration, int num) {
		...
		int height = appiumDriver.manage().window().getSize().getHeight();
		for (int i = 0; i < num; i++) {
			appiumDriver.swipe(width * 3 / 4, height / 2, width / 4, height / 2, duration);
			sleep(1000);
		}
	}
	...
	public void appSwipeRight(int duration, int num) {
		...
		int height = appiumDriver.manage().window().getSize().getHeight();
		for (int i = 0; i < num; i++) {
			appiumDriver.swipe(width / 4, height / 2, width * 3 / 4, height / 2, duration);
			sleep(1000);
		}
	}
    
	/**
	 * 点击坐标（x,y）duration毫秒后松开，反复fingers次
	 * 
	 * @author lijialong
	 */
	@Override
	...
	public boolean getToastText(String str, int sec) {
		boolean flag = false;
		AppiumDriver<?> appiumDriver = getAppDriver();
		WebDriverWait wait = new WebDriverWait(appiumDriver, sec);
		try {
			wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath(".//*[contains(@text,'" + str + "')]")));
			System.out.println(appiumDriver.findElement(By.xpath(".//*[contains(@text,'" + str + "')]")).getText());
			flag = true;
		} catch (Exception e) {
			log.info("toast: " + str + " not fund");
		}
		return flag;
	}
